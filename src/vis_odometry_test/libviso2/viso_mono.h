...
////        os << static_cast<const VisualOdometry::parameters &>(parameters);
//        return os;
//    }

    friend std::ostream &operator<<(std::ostream &os, const parameters &parameters) {
        os << " height: " << parameters.height
           << " pitch: " << parameters.pitch
           << " ransac_iters: " << parameters.ransac_iters
           << " inlier_threshold: " << parameters.inlier_threshold
           << " motion_threshold: " << parameters.motion_threshold;
        return os;
    }
...
private:
    ...
    void
    fundamentalMatrix(const std::vector<Matcher::p_match> &p_matched, const std::vector<int32_t> &active, Matrix &F);
